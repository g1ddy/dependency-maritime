            ...
            <div className="absolute left-5 right-5 mt-1 bg-gray-700 border border-gray-600 rounded shadow-xl max-h-48 overflow-y-auto text-sm z-50">
              {searchResults.length > 0 ? searchResults.map(node => (
                <button
                  key={node.id}
                  type="button"
                  className="w-full text-left px-3 py-2 hover:bg-gray-600 text-gray-200 border-b border-gray-600 last:border-0 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500"
                  onClick={() => {
                    selectNode(node.id);
                    setSearchQuery("");
                  }}
