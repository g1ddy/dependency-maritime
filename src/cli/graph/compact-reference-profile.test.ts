...
const dependency = (resolved: string, overrides: Partial<MaritimeDependency> = {}): MaritimeDependency => ({
    circular: false, coreModule: false, couldNotResolve: false, dependencyTypes: ['local', 'import'],
    dynamic: false, exoticallyRequired: false, followable: true, module: resolved,
    moduleSystem: 'es6', resolved, valid: true, ...overrides
});
    ...
    return {
        modules: [
            { source: 'src/features/a.ts', valid: true, dependents: [], dependencies: [
                ...
                dependency('src/types/c.ts', { dependencyTypes: ['local', 'type-only', 'pre-compilation-only'], typeOnly: true, preCompilationOnly: true })
            ] },
...
describe('architecture presentation profiles', () => {
    it('compact retains file-level LR topology with architecture presentation', () => {
    ...
    it('keeps a pair primary when runtime and pre-compilation evidence share endpoints', () => {
        const graph = fixture();
    ...
    it('has no hidden compact-profile behavior: equivalent settings reproduce it', () => {
        ...
        const explicit = renderDependencyGraphToDot(graph, {
            externalPackages: 'none', folderGrouping: 'nested', edgeLabels: 'none', layoutDirection: 'lr',
            rankConstraints: 'all', layoutDensity: 'compact', moduleAggregation: 'none', visualTheme: 'architecture',
            sourceRootGrouping: 'elide-single', edgePresentation: 'semantic-pairs', clusterRanking: 'local', aggregationDepth: 2
        }, { sourceRoots: ['src', 'tools'] });
        expect(explicit).toBe(compact);
    });

    it('architecture-overview is the explicit folder-summary intent', () => {
