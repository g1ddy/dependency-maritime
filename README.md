...
### Bring Your Own Data
Easily upload your `dependency-graph.json` file generated by Maritime or raw `dependency-cruiser` JSON output to visualize your codebase.
...
* **Local-First Processing:** Code analysis and graph visualisations run entirely within your local environment or CI runner. No source code or metrics are transmitted to any Maritime-hosted external service (though CI workflows may upload generated evidence to your configured CI provider).
...
```bash
npm install --save-dev @dependency-maritime/cli@prerelease
...
```yaml
- uses: g1ddy/dependency-maritime@cli-v0.1.0-beta.9
  with:
    source-roots: src
    render-graph: 'true'
...
Open `http://localhost:5173` in your browser, then click the **Upload** button to load a `dependency-graph.json` file.
