...
describe('composite action release tag resolution', () => {
    it('keeps rendering opt-in and resolves its input and output paths', () => {
        ...
        expect(parsed.inputs['external-packages']?.default).toBe('direct');
        expect(parsed.inputs['folder-grouping']?.default).toBe('nested');
        expect(parsed.inputs['edge-labels']?.default).toBe('types');
